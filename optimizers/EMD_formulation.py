...
def run_model(workers, requesters, state_probabilities):
	...
	@lru_cache(maxsize=num_state_combinations)
	def sc_probability(s):
		prod = 1
		s_bar = state_combinations[s]
	...
	m.addConstrs((delay(r, w, state_combinations[s][w]) <= c[r][s] for (r, w, s) in rws_combinations) , 'c1.0')
	# c1 means the expected time delay must not exceed the deadline of requester i
	m.addConstrs(( gurobi.quicksum([ sc_probability(s)*(c[r][s]) for s in range(num_state_combinations) ]) <= requesters[r].T for r in range(num_requesters) ), 'c1.1')
	...
	EMD_objective = gurobi.quicksum([ sc_probability(s)*(c[r][s]) for r, s in rs_combinations ])
	...
	# for s in range(num_state_combinations):
	# 	for w in range(num_workers):
	# 		print(c[w][s].X, end=' | ')

	# 	print()

	return association, allocation, EMD_objective.getValue()