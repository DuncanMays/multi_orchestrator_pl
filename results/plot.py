...
target_files = [os.path.join(src_dir, name) for name in target_files]

# this dict holds the average value for all data points accross the training runs recorded in src_dir
averages = {}
num_training_runs = len(target_files)

def get_averages(file_names):
...
def main():
	averages = get_averages(target_files)
