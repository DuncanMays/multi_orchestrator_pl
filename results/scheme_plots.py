...
target_dir = './vary_budget'
schemes = ['ideal_TT', 'ideal_EOL', 'uncertain_TT', 'uncertain_EOL']

def get_data(midstring):
	...
	for scheme_name in schemes:
		file_name = scheme_name+'_'+midstring+'_budget_0.json'
...
budget_settings = [(55, 60), (60, 65), (65, 70), (70, 75), (75, 80)]
metric = 'training_time'

line_styles = {'ideal_TT': '-', 'ideal_EOL': '-o', 'uncertain_TT': '--', 'uncertain_EOL': '--o'}

Y = {scheme: [] for scheme in schemes}
x = [i for i in range(len(budget_settings))]

for n in budget_settings:

	midstr = str(n[0])+'_'+str(n[1])

	avg_data = get_data(midstr)
...
plt.title('Training Time versus Budget', size=30)
plt.ylabel('Training Time (s)', size=20)
plt.legend(prop={'size': 20})
plt.xlabel('Budget (mnist_ffn, mnist_cnn)', size=20)
plt.xticks(x, [str(t) for t in budget_settings], size=15)
