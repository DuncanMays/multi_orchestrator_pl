...
if (__name__ == '__main__'):
	...
	state_probabilities = [get_state_distribution() for _ in range(num_learners)]

	worker_prices = [max(random.gauss(config.worker_price_variance, config.worker_price_mean), 0) for _ in range(num_learners)]

	# association, allocation, time_predictions = MMTT(benchmark_scores, worker_prices, state_probabilities, tasks)
	association, allocation, time_predictions = MED(benchmark_scores, worker_prices, state_probabilities, tasks)
